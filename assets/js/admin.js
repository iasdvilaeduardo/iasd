...
async function showDashboard(user) {
  ...
  const openPhotoManager = async (albumId) => {
    ...
    $('#photoAdminList').innerHTML = error ? '<p>Não foi possível carregar as fotos.</p>' : photos.length ? photos.map((photo) => `<article><img src="${storageUrl(photo.storage_path)}" alt="${escapeHtml(photo.alt_text || '')}"><button type="button" data-delete-photo="${photo.id}" data-path="${escapeHtml(photo.storage_path)}" data-album="${albumId}">Remover foto</button></article>`).join('') : '<p>Este álbum ainda não tem fotos.</p>';
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (photo) { if (!confirm('Remover somente esta foto?')) return; const albumId = event.target.dataset.album; const path = event.target.dataset.path; const storageError = (await supabase.storage.from('gallery').remove([path])).error; const dbError = storageError ? storageError : (await supabase.from('photos').delete().eq('id', photo)).error; if (!dbError) { const [{ data: albumData }, { data: remainingPhotos }] = await Promise.all([supabase.from('albums').select('cover_path').eq('id', albumId).single(), supabase.from('photos').select('storage_path').eq('album_id', albumId).order('position').limit(1)]); if (albumData?.cover_path === path) await supabase.from('albums').update({ cover_path: remainingPhotos?.[0]?.storage_path || null }).eq('id', albumId); } status(dbError ? 'Não foi possível remover a foto.' : 'Foto removida.'); if (!dbError) openPhotoManager(albumId); return; }
