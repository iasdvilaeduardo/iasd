...
async function showDashboard(user) {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (photo) { if (!confirm('Remover somente esta foto?')) return; const storageError = (await supabase.storage.from('gallery').remove([event.target.dataset.path])).error; const dbError = storageError ? storageError : (await supabase.from('photos').delete().eq('id', photo)).error; status(dbError ? 'Não foi possível remover a foto.' : 'Foto removida.'); if (!dbError) openPhotoManager(event.target.dataset.album); return; }
